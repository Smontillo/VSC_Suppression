...
def dev_scan(std_comb, TaskArray, nrank, omega_s, Psis, Ps, Nq, WQ, dw):
    ...
    for n in range(comb_len):
        ...
        Cq = np.ones(Nq, dtype = np.complex64) * (cQ/Nq**0.5).astype(np.complex64)
    # RPV - cavity distribution    
        stdD = std_comb[TaskArray[n]]  * (np.pi)
...
std_dip = np.array([1/10, 1/2, 1]) * np.pi

tot_Tasks = len(std_dip)
...
ini_time = time.time()
Jeff = dev_scan(std_dip, TaskArray, nrank, omega_s, Psis, Ps, Nq, WQ, dw)
fin_time = time.time()
print("Time again = ",fin_time - ini_time)

np.savetxt(f'./Jeff_{nrank}.txt', np.c_[omega_s,Jeff])
