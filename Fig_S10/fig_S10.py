...
#====================================
freq = len(np.loadtxt('Fig_S10a_data/Jeff_0.txt')[:,0])
Jeff = np.zeros((freq,ntasks))
color = ['b', 'green', 'red', 'blueviolet', 'darkorange']

fig, ax = plt.subplots(figsize = (4.5,4.5))
for k in range(ntasks):
    data = np.real(np.loadtxt(f'Fig_S10a_data/Jeff_{k}.txt', dtype = complex))
...
plt.savefig('Fig_S10a.pdf', dpi = 500, bbox_inches='tight')
plt.close()
...
for k in range(ntask):
    data = np.array(np.loadtxt(f'Fig_S10b_data/rate_{k}.txt'))
...
plt.savefig('Fig_S10b.pdf', dpi = 500, bbox_inches='tight')
plt.close()
