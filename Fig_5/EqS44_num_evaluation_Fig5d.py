...
def sigma(w):
    ...
    # f = (2 * λv * γv * w_cut)/(w_cut**2 + γv**2) * np.cosh(beta*w_cut/2)/np.sinh(beta*w_cut/2)
...
def dev_scan(std_comb, TaskArray, nrank, omega_s, Psis, Ps, Nq, WQ, dw):
    ...
    # RPV - cavity distribution    
        stdD = std_comb[TaskArray[n],1]  * (np.pi)
        dist = np.real(np.random.uniform(0, stdD, Nq)) 
        cos_dist = np.cos(dist)
...
std_comb = np.array(list(itertools.product(std_cQ, std_dip)))
...
ini_time = time.time()
rates, task_comb = dev_scan(std_comb, TaskArray, nrank, omega_s, Psis, Ps, Nq, WQ, dw)
fin_time = time.time()
print("Time again = ",fin_time - ini_time)

cQ_comb = std_comb[TaskArray[:],0]
dip_comb = std_comb[TaskArray[:],1]

np.savetxt(f'../data11/rate_{nrank}.txt', np.c_[cQ_comb,dip_comb,rates])
