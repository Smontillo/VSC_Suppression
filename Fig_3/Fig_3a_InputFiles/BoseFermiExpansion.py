...
def MSD (N,BoseFermi=1):
    ...
    if BoseFermi == 1:
        pole = np.array([2*(i+1)*pi for i in range(N)])
...
def PSD(N, BoseFermi=1, pade=2):
    ...
    """ 
        PSD(N, BoseFermi, pade)
        
        N = number of terms;
        BoseFermi = 1 for bosonic bath; BoseFermi = 2 for fermionic bath;
        pade = 0 for Matsubara decomposition; 
        pade = 1, 2, 3 for [N-1/N], [N/N], [N+1/N] type decomposition, respectively.
    """
    ...
    # [N-1/N], [N/N] type decomposition
        ...
        if N > 0:
            ...
            doff = np.array([1.0/sqrt((temp+2.0*i)*(temp+2.0*(i+1))) for i in range(M-1)])
            pole = 2.0/tseig(diag,doff)[0:N]
            ...
            M = int(M/2)  
            eig2 = np.power(2.0/tseig(diag,doff)[0:M],2)
            scaling = 0.0
    ...
    # [N+1/N] type decomposition
        ...
        M = 2*N+1
        diag = np.zeros(M,dtype=float) 
        doff = np.array([1.0/sqrt(d[i+1]*d[i+2]) for i in range(M-1)])
        pole = 2.0/tseig(diag,doff)[0:N]
        resi = np.zeros(N,dtype=float)
        for j in range(N):
...
if __name__ == '__main__':
    ...
    pole, resi, rn, tn = PSD(8,1,2)
#    np.savetxt("pole.txt", pole)
#    np.savetxt("resi.txt", resi)
    print(pole, resi)